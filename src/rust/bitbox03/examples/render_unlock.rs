...
//! Headless renderer for the unlock animation, for visual review of the baked lock frames through
//! LVGL's real software renderer (no GPU / windowing needed). Renders on the real 480×800 geometry.
//!
//! ```sh
//! # Render a single frame (index 0..=19; default 19, the fully open lock):
//! cargo run -p bitbox03 --example render_unlock -- /tmp/unlock.bmp 6
//! sips -s format png /tmp/unlock.bmp --out /tmp/unlock.png   # macOS; or ImageMagick `convert`
//! ```
...
use bitbox03::ui::unlock_animation;
...
fn main() {
    let out_path = std::env::args()
        .nth(1)
        .unwrap_or_else(|| "unlock_preview.bmp".to_string());
    let frame: usize = std::env::args()
        .nth(2)
        .and_then(|s| s.parse().ok())
        .unwrap_or(unlock_animation::FRAME_COUNT - 1);
    ...
    eprintln!("wrote {out_path} (frame {frame})");
}
